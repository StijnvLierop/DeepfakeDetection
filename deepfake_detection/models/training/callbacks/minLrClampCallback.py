...
class MinLrClampCallback(TrainerCallback):
    ...
    def on_step_end(self, args, state, control, **kwargs):
        ...
        if changed and state.global_step > 0 and state.global_step % args.logging_steps == 0:
            logger.info("Clamped learning rate to min_lr=%s at step=%s", self.min_lr, state.global_step)
        return control
