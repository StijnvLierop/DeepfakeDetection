...
def test_split_dataset_stratified_authenticity_label(dummy_dataset: Dataset):
    train_set, test_set = split_dataset(dummy_dataset, test_size=0.2,
                                        random_state=42, stratify=True, label_type='authenticity_label')
    counter_train = collections.Counter([instance.annotation.authenticity_label for instance in train_set])
    counter_test = collections.Counter([instance.annotation.authenticity_label for instance in test_set])
    counter_original = collections.Counter([instance.annotation.authenticity_label for instance in dummy_dataset])
...
def test_split_dataset_stratified_source_label(dummy_dataset: Dataset):
    train_set, test_set = split_dataset(dummy_dataset, test_size=0.2, random_state=42, stratify=True)
    counter_train = collections.Counter([instance.annotation.source_label for instance in train_set])
    counter_test = collections.Counter([instance.annotation.source_label for instance in test_set])
    counter_original = collections.Counter([instance.annotation.source_label for instance in dummy_dataset])
    for label in counter_original:
        train_ratio = counter_train[label] / len(train_set)
        test_ratio = counter_test[label] / len(test_set)
        original_ratio = counter_original[label] / len(dummy_dataset)
