...
from deepfake_detection.data.datasets.huggingface import HuggingfaceDataset, _build_hf_filter
...
def test_hf_filter_inline_func_shorthand():
    ...
    f = _build_hf_filter({
        "deepfake_detection.data.utils.filter.hf_hash_filter": {
            "column": "filename", "range_min": 0.0, "range_max": 1.0,
        }
    })
...
def test_take_on_map_dataset():
    ds = HuggingfaceDataset(
        dataset=MockMapDataset(_ROWS), instance_col="image", take=2
    )
    assert len(ds) == 2
